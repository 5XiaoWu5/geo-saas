...
export default function ApiKeysPage() {
  return (
    <div>
      <PageHeader title="API Keys" description="Static API key management shell. No keys are generated and no APIs are called." action={<Button><Plus className="h-4 w-4" /> New key</Button>} />
        ...
        {apiKeys.map((key) => (
          <Card key={key.prefix} className="glass-panel border-white/10">
            <CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <span className="text-sm text-muted-foreground">Last used {key.lastUsed}</span>
            </CardContent>
          </Card>
        ))}
      </div>
