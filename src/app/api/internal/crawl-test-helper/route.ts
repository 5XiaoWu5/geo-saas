...
const allowedPattern = /^geopilot-crawl-\d+@example\.com$/;

function validateEmail(email: unknown) {
  if (typeof email !== "string" || !allowedPattern.test(email)) throw new Error("只允许本次抓取测试邮箱");
  return email;
}
...
export async function POST(request: Request) {
  ...
  if (action === "cleanup") {
    ...
    for (const user of users) {
      ...
      const projects = await sql.query('SELECT "id" FROM "Project" WHERE "userId" = $1', [userId]);
      for (const project of projects) {
        deletedScans += (await sql.query('DELETE FROM "WebsiteScan" WHERE "projectId" = $1 RETURNING "id"', [String(project.id)]).catch(() => [])).length;
      }
    ...
    return NextResponse.json({ ok: true, deletedScans, deletedProjects, deletedUsers });
  }
