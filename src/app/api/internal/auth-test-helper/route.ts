...
const allowedPattern = /^geopilot-isolation-[a-z]-\d+@example\.com$/;

function validateEmail(email: unknown) {
  if (typeof email !== "string" || !allowedPattern.test(email)) {
    throw new Error("只允许本次隔离测试邮箱");
  }
  return email;
}
...
export async function POST(request: Request) {
  ...
  if (action === "cleanup") {
    const users = await sql.query('SELECT "id", "email" FROM "User" WHERE "email" = $1', [targetEmail]);
