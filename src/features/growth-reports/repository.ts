...
export const growthReportRepository = {
  ...
  async loadEvidence(userId: string, projectId: string): Promise<GrowthReportEvidence | null> {
    ...
    const settled = await Promise.allSettled([
      ...
      db.query('SELECT vc."id", vc."provider", vc."prompt", vc."answer", vc."brandMentioned", vc."mentionPosition", vc."sourceUrls", vc."score", vc."createdAt" FROM "VisibilityCheck" vc INNER JOIN "VisibilityCampaign" campaign ON campaign."id" = vc."campaignId" WHERE campaign."projectId" = $1 ORDER BY vc."createdAt" DESC LIMIT 100', [projectId]),
      db.query('SELECT citation."id", citation."checkId", citation."url", citation."domain", citation."position", citation."metadata", citation."createdAt" FROM "VisibilityCitation" citation INNER JOIN "VisibilityCheck" vc ON vc."id" = citation."checkId" INNER JOIN "VisibilityCampaign" campaign ON campaign."id" = vc."campaignId" WHERE campaign."projectId" = $1 ORDER BY citation."createdAt" DESC LIMIT 300', [projectId]),
