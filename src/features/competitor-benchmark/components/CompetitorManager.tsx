...
export function CompetitorManager({ projectId }: { projectId: string }) {
  ...
  const [pendingDelete, setPendingDelete] = useState<CompetitorProfile | null>(null);
  ...
  return (
    <div className="min-w-0 space-y-6 overflow-x-hidden">
      ...
      <Dialog open={Boolean(pendingDelete)} onOpenChange={open => { if (!open && !deletingId) setPendingDelete(null); }}>
        <DialogContent>
          <DialogTitle className="pr-12 text-lg font-semibold">{t("competitors.delete")}</DialogTitle>
          <DialogDescription className="mt-3 text-sm leading-6 text-muted-foreground">
            {t("competitors.deleteConfirm").replace("{name}", pendingDelete?.name ?? "")}
          </DialogDescription>
          <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <DialogClose asChild><Button variant="outline" className="min-h-11">{t("common.cancel")}</Button></DialogClose>
            <Button variant="destructive" className="min-h-11" disabled={!pendingDelete || Boolean(deletingId)} onClick={() => pendingDelete && void remove(pendingDelete)}>
              {deletingId ? <Loader2 className="h-4 w-4 animate-spin motion-reduce:animate-none" /> : <Trash2 className="h-4 w-4" />}
