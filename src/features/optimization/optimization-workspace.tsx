...
function ExecutionPanel({ execution, locale = "zh" }: { execution: ExecutionFeedback; locale?: "zh" | "en" }) {
  const sharedStatus = execution.status === "completed" ? "COMPLETED" : execution.status === "failed" ? "FAILED" : "RUNNING";
  return <Card className={`glass-panel ${execution.status === "failed" ? "border-rose-300/30" : execution.status === "completed" ? "border-emerald-300/30" : "border-violet-300/30"}`}><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle className="text-lg">{locale === "zh" ? "优化执行过程（Execution Timeline）" : "Execution Timeline"}</CardTitle><p className="mt-2 text-sm text-muted-foreground"><strong className="text-foreground">{locale === "zh" ? "发现问题" : "Detected issue"}：</strong>{execution.issue}</p><p className="mt-1 text-sm text-muted-foreground"><strong className="text-foreground">{locale === "zh" ? "准备执行" : "Operation"}：</strong>{execution.operation}</p></div><Badge variant={execution.status === "completed" ? "success" : execution.status === "failed" ? "warning" : "outline"}>{execution.status}</Badge></div></CardHeader><CardContent><OperationFeedback status={sharedStatus} message={execution.status === "completed" ? (locale === "zh" ? "真实任务记录已保存并重新加载。" : "The persisted task record was saved and reloaded.") : undefined} className="mb-4" /><ol className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{execution.steps.map((step, index) => { const Icon = step.status === "completed" ? CheckCircle2 : step.status === "failed" ? XCircle : step.status === "running" ? Loader2 : Circle; const englishLabels = ["Read current data", "Generate optimization plan", "Create or update task", "Record execution result"]; return <li key={`${step.label}-${index}`} className="min-w-0 rounded-2xl border border-white/10 p-4"><div className="flex items-center gap-2"><Icon className={`h-4 w-4 shrink-0 ${step.status === "running" ? "animate-spin text-violet-300 motion-reduce:animate-none" : step.status === "completed" ? "text-emerald-300" : step.status === "failed" ? "text-rose-300" : "text-muted-foreground"}`} /><span className="text-xs text-muted-foreground">Step {index + 1}</span></div><p className="mt-3 text-sm font-medium">{locale === "zh" ? step.label : englishLabels[index]}</p>{step.detail ? <p className="mt-2 break-words text-xs text-muted-foreground">{step.detail}</p> : null}</li>; })}</ol>{execution.result ? <div className="mt-4 rounded-2xl border border-emerald-300/20 bg-emerald-300/[0.04] p-4"><p className="text-sm font-medium text-emerald-200">{locale === "zh" ? "已创建或更新优化任务" : "Optimization Task created or updated"}</p><p className="mt-2 break-all text-xs text-muted-foreground">{locale === "zh" ? "关联记录" : "Related record"}：{execution.result.taskId}</p><p className="mt-1 text-xs text-muted-foreground">{locale === "zh" ? "影响模块" : "Affected modules"}：{execution.result.modules.join(" · ")}</p></div> : null}</CardContent></Card>;
}
