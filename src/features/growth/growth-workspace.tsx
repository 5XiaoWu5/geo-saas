...
export function GrowthWorkspace({ initialProjectId, view = "timeline" }: { initialProjectId?: string; view?: "overview" | "timeline" }) {
  ...
  return (
    <div className="min-w-0 space-y-6">
      <PageHeader title={view === "overview" ? "企业增长总览" : "企业增长中心"} description="统一查看 SEO、AI 搜索、企业知识与竞争增长，并保持底层增长引擎独立。" action={<div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row"><Button asChild variant="outline" className="min-h-11"><Link href={activeProject ? `/projects/${activeProject.id}/growth` : "/growth"}>增长时间线 <TrendingUp className="h-4 w-4" /></Link></Button><Button asChild className="min-h-11"><Link href={activeProject ? `/projects/${activeProject.id}/optimization` : "/optimization"}>进入优化中心 <ArrowRight className="h-4 w-4" /></Link></Button></div>} />
      ...
      {!data?.projects.length ? <Card className="glass-panel border-white/10"><CardContent className="p-6"><p className="text-sm text-muted-foreground">{t("growth.noProjects")}</p><Button asChild className="mt-4"><Link href="/projects">{t("growth.goToProjects")}</Link></Button></CardContent></Card> : activeProject ? <>
        ...
        <GrowthEngineBridge projectId={activeProject.id} />
...
function GrowthEngineBridge({ projectId }: { projectId: string }) {
  const flows = [
    { icon: SearchCheck, title: "SEO Growth Engine", target: "搜索引擎理解", steps: ["WebsiteScan", "Technical SEO / Content / Schema", "索引与自然搜索增长"], href: `/projects/${projectId}/seo`, color: "text-emerald-300", border: "border-emerald-300/20" },
    { icon: Bot, title: "AI Search Growth Engine", target: "AI 模型理解", steps: ["Website Content + Knowledge Assets", "Company Knowledge Profile", "GEO / Visibility / 推荐增长"], href: `/projects/${projectId}/geo`, color: "text-violet-300", border: "border-violet-300/20" },
  ];
  return <section className="grid gap-4 lg:grid-cols-2">{flows.map(({ icon: Icon, title, target, steps, href, color, border }) => <Card key={title} className={`glass-panel min-w-0 ${border}`}><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><div><p className={`flex items-center gap-2 font-semibold ${color}`}><Icon className="h-5 w-5" />{title}</p><p className="mt-2 text-sm text-muted-foreground">负责{target}</p></div><Globe2 className="h-5 w-5 text-muted-foreground" /></div><div className="mt-5 flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center">{steps.map((step, index) => <div key={step} className="contents"><div className="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/[0.025] px-3 py-3 text-xs">{step}</div>{index < steps.length - 1 ? <ArrowRight className="h-4 w-4 shrink-0 rotate-90 text-muted-foreground sm:rotate-0" /> : null}</div>)}</div><Button asChild variant="outline" className="mt-5 min-h-11 w-full"><Link href={href}>进入增长引擎 <ArrowRight className="h-4 w-4" /></Link></Button></CardContent></Card>)}</section>;
}
