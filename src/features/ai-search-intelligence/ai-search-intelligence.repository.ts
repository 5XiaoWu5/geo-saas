...
async function evidenceQuery(source: string, query: string, params: unknown[]) {
  try { return await aiSearchDatabase().query(query, params); }
  catch (error) {
    console.error(`[AI SEARCH INTELLIGENCE] ${source} evidence query failed`, error instanceof Error ? error.message : String(error));
    throw new Error(`AI_SEARCH_EVIDENCE_${source}_FAILED`);
  }
}
...
export const aiSearchIntelligenceRepository = {
  ...
  async loadEvidence(userId: string, projectId: string, provider: string): Promise<AIRecommendationEvidence> {
    const [entityRows, knowledgeRows, productRows, caseRows, technicalRows, visibilityRows, benchmarkRows, simulationRows] = await Promise.all([
      evidenceQuery("ENTITY", 'SELECT entity.* FROM "EntityProfile" entity INNER JOIN "Project" p ON p."id" = entity."projectId" WHERE entity."projectId" = $1 AND p."userId" = $2 ORDER BY entity."updatedAt" DESC LIMIT 1', [projectId, userId]),
      evidenceQuery("KNOWLEDGE", 'SELECT kb."id" AS "baseId", kb."version", kb."completenessScore", profile."id" AS "profileId", profile."missingKnowledge", profile."certifications", profile."faqTopics" FROM "CompanyKnowledgeBase" kb INNER JOIN "Project" p ON p."id" = kb."projectId" LEFT JOIN "CompanyKnowledgeProfile" profile ON profile."projectId" = kb."projectId" WHERE kb."projectId" = $1 AND p."userId" = $2 LIMIT 1', [projectId, userId]),
      evidenceQuery("PRODUCT", 'SELECT product.* FROM "ProductEntity" product INNER JOIN "Project" p ON p."id" = product."projectId" WHERE product."projectId" = $1 AND product."status" = \'ACTIVE\' AND p."userId" = $2 ORDER BY product."updatedAt" DESC', [projectId, userId]),
      evidenceQuery("CUSTOMER_PROOF", 'SELECT customer_case.* FROM "CustomerCase" customer_case INNER JOIN "Project" p ON p."id" = customer_case."projectId" WHERE customer_case."projectId" = $1 AND customer_case."status" = \'ACTIVE\' AND p."userId" = $2 ORDER BY customer_case."updatedAt" DESC', [projectId, userId]),
      evidenceQuery("TECHNICAL", 'SELECT technical.* FROM "TechnicalDocument" technical INNER JOIN "Project" p ON p."id" = technical."projectId" WHERE technical."projectId" = $1 AND technical."status" = \'ACTIVE\' AND p."userId" = $2 ORDER BY technical."updatedAt" DESC', [projectId, userId]),
      evidenceQuery("VISIBILITY", 'SELECT visibility_check."id", visibility_check."score", (CARDINALITY(visibility_check."sourceUrls") > 0 OR COUNT(citation."id") > 0) AS "cited", COALESCE(ARRAY_AGG(citation."id") FILTER (WHERE citation."id" IS NOT NULL), ARRAY[]::text[]) AS "citationIds" FROM "VisibilityCheck" visibility_check INNER JOIN "VisibilityCampaign" campaign ON campaign."id" = visibility_check."campaignId" INNER JOIN "Project" p ON p."id" = campaign."projectId" LEFT JOIN "VisibilityCitation" citation ON citation."checkId" = visibility_check."id" WHERE campaign."projectId" = $1 AND p."userId" = $2 GROUP BY visibility_check."id" ORDER BY visibility_check."createdAt" DESC LIMIT 100', [projectId, userId]),
      evidenceQuery("BENCHMARK", 'WITH latest_run AS (SELECT run."id" FROM "BenchmarkRun" run INNER JOIN "Project" p ON p."id" = run."projectId" WHERE run."projectId" = $1 AND run."status" = \'COMPLETED\' AND p."userId" = $2 ORDER BY run."createdAt" DESC LIMIT 1) SELECT result.*, latest_run."id" AS "runId" FROM latest_run INNER JOIN "BenchmarkResult" result ON result."benchmarkRunId" = latest_run."id" ORDER BY result."ranking" ASC NULLS LAST', [projectId, userId]),
      evidenceQuery("SIMULATION", 'SELECT result.*, task."query", task."provider" FROM "SimulationResult" result INNER JOIN "SimulationTask" task ON task."id" = result."taskId" INNER JOIN "Project" p ON p."id" = task."projectId" WHERE task."projectId" = $1 AND task."targetType" = \'OWN\' AND task."provider" = $3 AND p."userId" = $2 ORDER BY result."createdAt" DESC LIMIT 1', [projectId, userId, provider]),
    ]);
  ...
  async syncOptimizationTasks(userId: string, projectId: string, issues: AIRecommendationIssue[]) {
    ...
    const now = new Date();
    return (await Promise.all(payload.map((item) => aiSearchDatabase().query('WITH owned_project AS (SELECT p."id" FROM "Project" p WHERE p."id" = $2 AND p."userId" = $10), updated AS (UPDATE "OptimizationTask" task SET "title" = $4, "description" = $5, "recommendation" = $6, "severity" = $7, "category" = $8, "updatedAt" = $9 FROM owned_project WHERE task."projectId" = owned_project."id" AND task."issueId" = $3 RETURNING task.*), inserted AS (INSERT INTO "OptimizationTask" ("id", "projectId", "issueId", "title", "description", "recommendation", "severity", "category", "status", "createdAt", "updatedAt") SELECT $1, owned_project."id", $3, $4, $5, $6, $7, $8, \'PENDING\', $9, $9 FROM owned_project WHERE NOT EXISTS (SELECT 1 FROM updated) RETURNING *) SELECT * FROM updated UNION ALL SELECT * FROM inserted', [item.id, projectId, item.issueId, item.title, item.description, item.recommendation, item.severity, item.category, now, userId])))).flat();
  },
};
