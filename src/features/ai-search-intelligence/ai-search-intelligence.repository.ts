...
export const aiSearchIntelligenceRepository = {
  ...
  async syncOptimizationTasks(userId: string, projectId: string, issues: AIRecommendationIssue[]) {
    ...
    const now = new Date();
    return (await Promise.all(payload.map((item) => aiSearchDatabase().query('INSERT INTO "OptimizationTask" ("id", "projectId", "issueId", "title", "description", "recommendation", "severity", "category", "status", "createdAt", "updatedAt") SELECT $1, p."id", $3, $4, $5, $6, $7, $8, \'PENDING\', $9, $9 FROM "Project" p WHERE p."id" = $2 AND p."userId" = $10 ON CONFLICT ("projectId", "issueId") DO UPDATE SET "description" = EXCLUDED."description", "recommendation" = EXCLUDED."recommendation", "severity" = EXCLUDED."severity", "category" = EXCLUDED."category", "updatedAt" = EXCLUDED."updatedAt" RETURNING *', [item.id, projectId, item.issueId, item.title, item.description, item.recommendation, item.severity, item.category, now, userId])))).flat();
  },
};
