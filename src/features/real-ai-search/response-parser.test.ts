...
test("只把 Provider 的结构化来源保存为引用", () => {
  ...
  assert.equal(result.mentioned, true); assert.equal(result.rankPosition, 1); assert.deepEqual(result.productMentions, ["Growth Engine"]); assert.deepEqual(result.competitorBrands, ["Rival One"]); assert.equal(result.citations.find((item) => item.domain === "geopilot.ai"), undefined); assert.equal(result.citations.find((item) => item.domain === "media.example")?.citationCount, 1);
});
